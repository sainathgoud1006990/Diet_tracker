import { pgTable, serial, date, text, integer, varchar, timestamp, unique } from "drizzle-orm/pg-core";
...
export const dietLogsTable = pgTable("diet_logs", {
  ...
  breakfastFood: text("breakfast_food"),
  lunchFood: text("lunch_food"),
  dinnerFood: text("dinner_food"),
  snacksFood: text("snacks_food"),
  ...
  unique("diet_logs_user_date_unique").on(table.userId, table.date),
]);
